...
function subtree0(&$SUBTREE, $node, &$R) {  //search in tree node1 and insert node2
    if(count($SUBTREE)) {
...
function subtree1(&$Item, &$SUBTREE, $node) { //search in tree node2 and insert node1 from tree
    if(count($SUBTREE)) {
        foreach($SUBTREE as $K=>$N) {
...
function countSub(&$SUBTREE, $cnt, &$REZ) { //calculate max inheriting
    if(count($SUBTREE)) {
        $cnt++;
